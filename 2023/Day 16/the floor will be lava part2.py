...
def energize_contraption(start_coords, direction):
    global move_made, max_energized, contraption, light_map
    light_map = []
    move_made = []
    energized_tiles = 0
    light_map = [["." for _ in range(len(contraption[0]))]
                 for _ in range(len(contraption))]

    make_a_step(start_coords, direction)

    for row in light_map:
        energized_tiles += row.count("#")
    print(start_coords, direction, energized_tiles)    
...
for i in range(len(contraption[0])):
    energize_contraption((i, -1), "DOWN")
    energize_contraption((i, len(contraption)), "UP")

for i in range(len(contraption)):
    energize_contraption((-1, i), "RIGHT")
    energize_contraption((len(contraption[0]), i), "LEFT")
